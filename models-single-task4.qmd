...
```{r}
x <- seq(-5, 5, 0.01)
y <- psgt(x, mu = -0.8, sigma = 0.9, lambda = 0, p = 4.8, q = 31.8, mean.cent = FALSE)
data.frame(x = x, y = y) %>% 
  ggplot(aes(x = x, y = y)) + 
  geom_line() # + 
  geom_vline(xintercept = -1.4, color = "gray", linetype = "dashed") + 
  geom_hline(yintercept = psgt(-1.4, mu = -1.4, sigma = 0.8, lambda = 0.2, p = 4, q = 1.7, mean.cent = FALSE))
```
