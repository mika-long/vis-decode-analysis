...
task_df %>% select(data.select.x, data.select.y, pixel.select.x, pixel.select.y) %>% 
  mutate(calculated.x = sapply(data.select.x, data_to_pixel_x), 
         calculated.y = sapply(data.select.y, data_to_pixel_y)) %>% 
  mutate(x_diff = round(pixel.select.x - calculated.x, 2), 
         y_diff = round(pixel.select.y - calculated.y, 2)) %>% 
  pull(y_diff)
```
...
Now that we know what the `pixelsPerMM` parameter is, we can write functions that transform pixel values into physical distances (measured in mm). Note that we need to do some transforms^[... specifically, distance to the x-axis would be the y-coordinate, which would be 410 - `pixel.select.y`. Distance to the y-axis would be the x-coordinate, which would be `pixel.select.x - margin.left`. ] because of the way direction works in svgs are counter-intuitive ...: 
...
task1_df <- task_df %>% filter(task == "task1") %>% 
  ...
  mutate(left_area = psgt(data.select.x, param.mu, param.sigma, 
...
Could the participants be using the highest point as a way to judge area? 
...
Could it be that the more symmetric a pdf looks, the more likely that its going to be "pulled" towards using mode as a substitute? 
...
task1_df %>% ggplot(aes(x = data.select.x, y = param.mu)) + 
  geom_point(alpha = 0.5) + 
  xlim(-1, 1) + 
  ylim(0, 1) + 
  geom_hline(yintercept = 0.5, linetype="dashed", color="gray") + 
...
task2_df <- task_df %>% filter(task == "task2") %>% 
  ...
  mutate(phy.select.x = pixel_to_phy_x(phy.ans.x, pixelToMM), 
         phy.select.y = pixel_to_phy_y(phy.ans.y, pixelToMM)) %>% 
  # compute visual angles 
...
First let's explore a bit --- distribution of signed error in difference spaces: 

```{r}
#| code-fold: true 
#| layout-ncol: 2

task2_df %>% ggplot(aes(x = data.select.x - data.ans.x)) + geom_dots()
task2_df %>% ggplot(aes(x = data.select.y - data.ans.y)) + geom_dots()

task2_df %>% ggplot(aes(x = pixel.select.x - pixel.ans.x)) + geom_dots()
task2_df %>% ggplot(aes(x = pixel.select.y - pixel.ans.y)) + geom_dots()

task2_df %>% ggplot(aes(x = phy.select.x - phy.ans.x)) + geom_dots()
task2_df %>% ggplot(aes(x = phy.select.y - phy.ans.y)) + geom_dots()

task2_df %>% ggplot(aes(x = va.select.x - va.ans.x)) + geom_dots()
task2_df %>% ggplot(aes(x = va.select.y - va.ans.y)) + geom_dots()
```

You see, it is with this physical distance that things don't quite make sense. How is it possible that the difference is 100mm = 10cm ...? 
...
task2_df %>% 
  ggplot(aes(y = data.select.grad, 
             x = param.p)) + 
  geom_point(alpha = 0.5) + 
  # another option
  # ggdist::geom_dots(layout="swarm") + 
  labs(title = "Task 2 - Find highest point - Data")

task2_df %>% 
  ggplot(aes(y = data.select.angle, 
             x = param.p)) + 
  geom_point(alpha = 0.5) + 
  labs(title = "Task 2 - Find highest point - Data")
```

There is the theoretical angle, which is just `atan(numDeriv::grad(selection)) * 180 / pi`. There is also the empirical angle, which is just `atan(diff.y / diff.x)`. 

```{r}
task2_df %>% mutate(data.diff.x = abs(data.select.x - data.ans.x), 
                    data.diff.y = abs(data.select.y - data.ans.y)) %>% 
  mutate(empirical_angle = atan(data.diff.y / data.diff.x) * 180 / pi) %>% 
  select(data.select.angle, empirical_angle, participantId) %>% 
  filter(empirical_angle <= 5) %>% 
  ggplot(aes(y = abs(data.select.angle), x = empirical_angle)) + 
  geom_point(alpha = 0.5) + 
...
Matt said that this result looks promising to him, because it shows that we can predict from visual angle to x-coordinate? Something of the sort. I need to be more clear on this. 

```{r}
#| code-fold: true 
task2_df %>% 
  ggplot(aes(y = data.select.x - param.mu, x = param.p)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title = "Task 2 - Find highest point - Data")

task2_df %>% mutate(ans.x = data_to_pixel_x(param.mu)) %>% 
  left_join(participants) %>% 
  ggplot(aes(y = phy.select.x - phy.ans.x, x = param.p)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title = "Task 2 - Find highest point - Data")
...
task2_df %>% 
  ggplot(aes(y = data.select.x - param.mu, x = param.q)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title = "Task 2 - Find highest point - Data")

task2_df %>% 
  ggplot(aes(y = data.select.grad, x = param.q)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title = "Task 2 - Find highest point - Angle")
```


We can also try to plot the distribution of the gradient at the selected point: 
...
task3_df <- task_df %>% filter(task == "task3") %>% 
  ...
  mutate(data.ans.y = 0.5, 
         pixel.ans.y = data_to_pixel_y(0.5), 
         phy.ans.y = pixel_to_phy_y(pixel.ans.y, pixelToMM), 
         va.ans.y = vis_angle(phy.ans.y, dist_to_screen))
...
# data space 
task3_df %>% ggplot(aes(x = data.select.x, y = data.select.y - 0.5)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title="Task 3 -- CDF Median")

# physical space
task3_df %>% ggplot(aes(x = phy.select.x, y = phy.select.y - phy.ans.y)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title="Task 3 -- CDF Median") 

# visual angle
task3_df %>% ggplot(aes(x = va.select.x, y = va.select.y - va.ans.y)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title="Task 3 -- CDF Median") 

# visual angle by participiant 
task3_df %>% ggplot(aes(x = va.select.x, y = va.select.y - va.ans.y)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title="Task 3 -- CDF Median") +
...
task4_df %>% 
  ggplot(aes(x = data.select.x, y = data.select.slope - data.ans.slope)) + 
  geom_point(alpha = 0.5) + 
  labs(title = "Task 4 -- Find Max Slope -- Data")

task4_df %>% 
  ggplot(aes(x = data.select.slope - data.ans.slope)) + 
  ggdist::geom_dots() + 
  labs(title = "Task 4 -- Find Max Slope -- Data")
...
task4_df %>% 
  ggplot(aes(x = data.select.slope - data.ans.slope)) + 
  # ggdist::geom_dots() + 
  ggdist::geom_swarm(side = "top", alpha = 0.5) + 
  labs(title = "Task 4 -- Find Max Slope -- Data") + 
  facet_wrap(~participantId)
```
...
#| code-fold: true
task4_df %>% 
  ggplot(aes(x = data.select.x, y = param.mu)) + 
...
# data space 
task5_df %>% ggplot(aes(y = data.select.x - data.ans.x, x = data.ans.y)) + 
  geom_point(alpha=0.5) + 
  geom_hline(yintercept = 0, color="gray", linetype="dashed") + 
  labs(title="Task 5 - Project from Dot to X-Axes - Data")

# screen space
task5_df %>% ggplot(aes(x = phy.ans.y, y = phy.select.x - phy.ans.x)) + 
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
...
# data space 
task5_df %>% ggplot(aes(y = data.select.y - data.ans.y, x = data.ans.x)) + 
  geom_point(alpha=0.5) + 
  geom_hline(yintercept = 0, color="gray", linetype="dashed") + 
  labs(title="Task 5 - Project from Dot to Y-Axes - Data")

# screen space
task5_df %>% ggplot(aes(x = phy.ans.x, y = phy.select.y - phy.ans.y)) + 
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
...
task5_df %>% ggplot(aes(x = va.ans.x, y = va.select.y - va.ans.y)) + 
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title="Task 5 - Project from Dot to Y-Axes - Vis Angle")
