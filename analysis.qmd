...
# data space 
task1_df %>% 
  ggplot(aes(x = param.lambda, y = location.x - param.mu)) + 
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title = "Task 1 -- Data")

# I don't think calculating the screen space matters here ...
...
task2_df %>% 
  ggplot(aes(y = grad.x, # location.x - ans.x, 
             x = param.p)) + 
  geom_point(alpha = 0.5) + 
...
task2_df %>% mutate(ans.x = data_to_pixel_x(param.mu)) %>% 
  ...
  ggplot(aes(y = pixel_to_phy_x(pixel.x, pixelToMM) - pixel_to_phy_x(ans.x, pixelToMM), x = param.p)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title = "Task 2 - Find highest point - Data")
