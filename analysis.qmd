...
task2_df %>% 
  ...
  ggplot(aes(x = x_diff)) + geom_density() + 
...
task2_df %>% 
  ...
  ggplot(aes(x = y_diff)) + geom_density() + 
  labs(title= "Task 2 (data space)")
```
...
```{r}
d <- task3_df %>% left_join(participants, by = join_by(participantId)) %>% 
...
Things track very well, except for some outliers^[Maryam took slope to mean the smallest value of x where it starts turning. I guess you could call it first kink? Or first x location where things start to ramp up in speed?]
...
task5_df %>% left_join(participants) %>% 
