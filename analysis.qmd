...
task2_df <- task_df %>% filter(task == "task2") %>% 
  ...
  mutate(data.ans.x = param.mu, 
         ...
         data.ans.grad = numDeriv::grad(dsgt, data.select.x, mu = param.mu, sigma = param.sigma, 
  ...
  mutate(va.select.x = vis_angle(phy.))

task2_df %>% head(5)

# dude this magnitude is weird ...
# maybe the pixel to physics thing is wrong ...? Like it requires the difference to be processed before ...? like what is the origin of this calculation? 
task2_df %>% ggplot(aes(x = phy.select.x - phy.ans.x, 
                        y = phy.select.y - phy.ans.y)) + 
  geom_point()
...
task2_df %>% 
  ggplot(aes(y = grad.x, # data.select.x - ans.x, 
             x = param.p)) + 
  geom_point(alpha = 0.5) + 
...
task2_df %>% 
  ggplot(aes(y = grad.x, 
             x = numDeriv::grad(dsgt, param.mu + 0.01, mu = param.mu, sigma = param.sigma, 
                                lambda = param.lambda, p = param.p, q = param.q, mean.cent = FALSE))) + 
  geom_point(alpha = 0.5) + 
  labs(title = "Task 2 - Find highest point - Data")
```

If we do things in the visual angle space then this is what we get: 

```{r}
task2_df %>% 
  ggplot(aes(y = atan(grad.x) * 180 / pi, 
             x = param.p)) + 
  geom_point(alpha = 0.5) + 
  labs(title = "Task 2 - Find highest point - Data")
```
...
task2_df %>% mutate(ans.x = data_to_pixel_x(param.mu)) %>% 
  ...
  ggplot(aes(y = pixel_to_phy_x(pixel.select.x, pixelToMM) - pixel_to_phy_x(ans.x, pixelToMM), x = param.p)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title = "Task 2 - Find highest point - Data")

task2_df %>% 
  ggplot(aes(y = grad.x * 180 / pi, x = param.p)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
...
task2_df %>% 
  ggplot(aes(y = grad.x * 180 / pi, x = param.q)) + 
  geom_point(alpha = 0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
  labs(title = "Task 2 - Find highest point - Angle")
```
...
task2_df %>% 
  ...
  mutate(x_diff = data.select.x - answer.x) %>% 
  ggplot(aes(x = x_diff)) + 
  # ggdist::geom_dots() + 
  ggdist::geom_swarm(side = "top") + 
...
task2_df %>% 
  mutate(answer.x = param.mu, 
         answer.y = dsgt(x=param.mu, mu = param.mu, sigma=param.sigma, 
                         lambda=param.lambda, p=param.p, q=param.q, mean.cent = F)) %>% 
  mutate(y_diff = data.select.y - answer.y) %>% 
  ggplot(aes(x = y_diff)) + 
  # ggdist::geom_dots() + 
  ggdist::geom_swarm(side = "top") + 
  labs(title= "Task 2 (data space)")
...
#| code-fold: true 
task2_df %>% 
  mutate(grad.x = numDeriv::grad(dsgt, data.select.x, mu = param.mu, sigma = param.sigma, 
                                 lambda = param.lambda, p = param.p, q = param.q, mean.cent=F), 
         grad.ans = 0,
         ) %>%
...
# data space 
task3_df %>% 
  ggplot(aes(x = data.select.x, y = data.select.y - 0.5)) + 
  geom_point() + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
...
task3_df %>% left_join(participants, by = join_by(participantId)) %>% 
  mutate(phy.x = pixel_to_phy_x(pixel.select.x, pixelToMM), 
         phy.y = pixel_to_phy_y(pixel.select.y, pixelToMM), 
...
task4_df <- task_df %>% filter(task == "task4") %>% 
  select(-slider.x, -slider.y) %>% 
  mutate(slope = dsgt(data.select.x, param.mu, param.sigma, param.lambda, param.p, param.q, mean.cent=FALSE), 
...
task4_df %>% 
  ggplot(aes(x = data.select.x, y = slope - ans)) + 
  geom_point(alpha = 0.5) + 
...
task5_df %>% 
  ggplot(aes(y = slider.x - data.select.x, x = data.select.y)) + 
  geom_point(alpha=0.5) + 
...
task5_df %>% left_join(participants, by = join_by(participantId)) %>% 
  mutate(phy.x = pixel_to_phy_x(data_to_pixel_x(slider.x), pixelToMM), 
         ans.phy.x = pixel_to_phy_x(data_to_pixel_x(data.select.x), pixelToMM),
         ans.phy.y = pixel_to_phy_y(data_to_pixel_y(data.select.y), pixelToMM)) %>%
...
task5_df %>% 
  ggplot(aes(x = data.select.x, y = slider.y - data.select.y)) + 
  geom_point(alpha=0.5) + 
  geom_hline(yintercept = 0, linetype="dashed", color="gray") + 
