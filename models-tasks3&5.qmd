...
The data looks kind of skewed ... but for now let's fit a normal model 

$$\begin{align*}
\texttt{error_va}_i &\sim \mathcal{N}(\mu, \sigma_i) \\ 
\sigma_i = \alpha \cdot \texttt{va.ans.y}_i &\leftrightarrow \log(\sigma_i) = \log(\alpha) + \log(\texttt{va.ans.y}_i)
\end{align*}$$
...
I also tried a skewed normal distribution with the same prior and compare the two using `loo`; the normal actually is better 
:::

```{r}
s.5.va %>% spread_draws(b_Intercept, b_sigma_Intercept) %>% 
...
$$\begin{aligned}
\text{error_phy}_i / \texttt{dist_to_screen}  &\sim \mathcal{N}(\mu, \sigma_{i}) \\ 
\mu &\sim \mathcal{N}(0, 1) \\ 
\sigma_{i} &= \beta \cdot (\texttt{phy.ans.y}_i) \cdot \hat{\sigma}, & j \in [13] \\ 
\hat{\sigma} &\sim \mathcal{N}(0, 1) & j \in [13]
\end{aligned}$$

`brms` formula: 

```{r}
f <- bf(error_phy.adj ~ 1, 
        sigma ~ 1 + offset(log(phy.ans.y)))
...
Check model fit and posterior: 

```{r}
summary(s.5.phy)
plot(s.5.phy)
pp_check(s.5.phy, ndraws = 100)
```
...
```{r}
f <- bf(error_va ~ 1 , 
        sigma ~ 0 + log(va.ans.x)) # what's the difference between this and 0 + offset(log(va.ans.x)) ??? 
