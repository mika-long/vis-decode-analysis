---
title: "Tasks 5 and 3 models (projections)"
format:
  html: 
    code-fold: false
...
task5_df <- single_pid_df %>% filter(task == "task5") %>% # or use task_df for full 
...
- Models can be fitted in visual angle space, as well as physical space. We will focus on fitting in visual angle space, for it is more uniform.  
- Within each space, the effect of "distance to the other axis" could be *multiplicative* or *additive*. 
...
\log(\sigma_i) &= \alpha + \log(\texttt{va.ans.y}_i) \\ 
\implies \sigma_i &= \exp(\alpha) \times \texttt{va.ans.y}_i \\ 
\alpha \sim N(0, 1)
\end{align*}$$
...
```{r}
#| output: false 
s.5.va <- task5_df %>% mutate(error_va = va.select.x - va.ans.x) %>%
...
summary(s.5.va)
plot(s.5.va)
pp_check(s.5.va, ndraws = 100)
```

Posterior of `exp(alpha)`: 

```{r}
# s.5.va %>% get_variables()

s.5.va %>% spread_draws(b_sigma_Intercept) %>% 
  mutate(b_sigma = exp(b_sigma_Intercept)) %>% 
  #pivot_longer(cols = c(b_Intercept, b_sigma)) %>% 
  ggplot(aes(x = b_sigma)) + 
  stat_halfeye() # + xlim(0, NA)
```
...
$$\begin{align*}
\texttt{error_va}_i &\sim \mathcal{N}(0, \sigma_{i, \text{PID}[i]}) \\ 
...
model_draws <- m.5.va %>%
  spread_draws(b_sigma_Intercept) %>%
  # Create a sequence of va.ans.y values to plot across
  mutate(va_ans_y = list(seq(2.40, 10.51, length.out = 100))) %>%
  unnest(va_ans_y) %>%
  # Calculate sigma for the average participant (using the log-linear model)
  mutate(
    log_sigma = b_sigma_Intercept + log(va_ans_y),
    sigma = exp(log_sigma)
  )

# Create the plot for the average participant
model_draws %>%
  ggplot(aes(x = va_ans_y, y = sigma)) +
  # Show individual posterior draws with transparency
  geom_line(aes(group = .draw), alpha = 0.05, color = "gray70") +
  # Add summary statistics
  stat_lineribbon(aes(y = sigma), .width = c(0.50, 0.80, 0.95), alpha = 0.3) +
  scale_fill_brewer(palette = "Blues") +
  labs(
    title = "Posterior distribution of sigma for an average participant",
    x = "Visual angle (va.ans.y)",
    y = "Sigma (standard deviation of error)",
