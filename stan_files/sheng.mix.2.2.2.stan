data {
  int<lower=1> N;   // total number of observations
  vector[N] x;      // observed values
  vector[N] x_med;  // known median values
  vector[N] x_mod;  // known mode values
}

parameters {
  // scaling factor for distance
  real beta;
  
  // standard deviations for x_med and x_mod
  real<lower=0> sigma_mod;
  real<lower=sigma_mod> sigma_med; // difference between sigmas
}

transformed parameters {
  vector<lower=0, upper=1>[N] lambda;

  for (n in 1:N) {
    // Calculate difference between mode and median
    real diff = x_med[n] - x_mod[n];

    lambda[n] = inv_logit(beta * abs(diff));
  }
}

model {
  // priors
  sigma_mod ~ normal(0.15, 0.01);
  sigma_med ~ normal(0.15, 0.6);

  beta ~ normal(0, 2);
  
  // likelihood
  for (n in 1:N) {
    // Two-component mixture model
    target += log_mix(lambda[n],
                      normal_lpdf(x[n] | x_med[n], sigma_med),
                      double_exponential_lpdf(x[n] | x_mod[n], sigma_mod));
  }
}
generated quantities {
  vector[N] log_lik; 
  vector[N] y_rep; // posterior predictive checks
  vector[N] x_org = x;
  
  for (n in 1:N) {
    log_lik[n] = log_mix(lambda[n],
                         normal_lpdf(x[n] | x_med[n], sigma_med),
                         double_exponential_lpdf(x[n] | x_mod[n], sigma_mod));

    // Generate posterior predictive samples
    if (bernoulli_rng(lambda[n]) == 1) {
      // Sample from normal component
      y_rep[n] = normal_rng(x_med[n], sigma_med);
    } else {
      y_rep[n] = double_exponential_rng(x_mod[n], sigma_mod);
    }
  }
}
