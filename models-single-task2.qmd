...
$$\begin{align*}\text{error}_i &\sim \text{Lorax}(\lambda, \alpha) \\ 
...
stan_funs <- "
  ...
  real custom_lomax_lpdf(real y, real mu, real lambda, real alpha) {
    return pareto_type_2_lpdf(y | 0, lambda, alpha);
  }
  
  real custom_lomax_rng(real mu, real lambda, real alpha) {
...
```{r}
#| output: false 
s.2.va.y.2 <- task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>%
...
```{r}
expose_functions(s.2.va.y.2, vectorize = TRUE)

# Define the posterior_predict function in R as per vignette
posterior_predict_custom_lomax <- function(i, draws, ...) {
  ...
  custom_lomax_rng(0, 1, alpha)
}

pp_check(s.2.va.y.2, ndraws = 100) + xlim(0, 0.5) # hmm can't really tell which one is better ... 
```

```{r}
s.2.va.y.2 %>% get_variables(.)

s.2.va.y.2 %>% spread_draws(b_lambda_Intercept, b_alpha_Intercept) %>% 
  mutate(lambda = exp(b_lambda_Intercept)) %>% 
...
stan_funs <- "
  ...
  real custom_pareto_lpdf(real y, real mu, real shape) {
    return pareto_lpdf(y | mu, shape);
  }
  
  real custom_pareto_rng(real mu, real shape) {
...
f <- bf(
  error.y ~ 1,  # Intercept-only model for mu (scale parameter)
  shape ~ 1  # Intercept-only model for shape parameter
)

p <- c(
    # Prior for the scale parameter (on log scale)
    prior(constant(log(7.028422e-11)), class = "Intercept"),
...
task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>% 
  get_prior(formula = f, family = pareto_family)

task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>% 
  make_stancode(formula = f, family = pareto_family, prior = p)
```
...
Define custom functions for `pp_check`: 
...
# Define the posterior_predict function in R as per vignette
posterior_predict_custom_pareto <- function(i, draws, ...) {
  mu <- draws$dpars$mu[i]
  shape <- draws$dpars$shape[i]
  # Use Stan's rng function indirectly via brms
  custom_pareto_rng(mu, shape)
}

pp_check(s.2.va.y.2, ndraws = 100) # + xlim(0, 1)
```

```{r}
log_lik_custom_pareto <- function(i, prep){
  mu <- brms::get_dpar(prep, "mu", i = i)
  shape <- brms::get_dpar(prep, "shape", i = i)
  y <- prep$data$Y[i]
  custom_pareto_lpdf(y, mu, shape)
}
```
...
```{r}
s.2.va.y.2 %>% spread_draws(b_shape_Intercept) %>% 
  mutate(alpha = exp(b_shape_Intercept)) %>% 
  ggplot(aes(x = alpha)) + 
  stat_halfeye()
```
...
```{r}
loo(s.2.va.y, s.2.va.y.2)
```

Hmm so the pareto distribution does actually fit better. 
