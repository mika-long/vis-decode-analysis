...
find_x_for_y_uniroot <- function(y_target, x_values, y_values, x_interval) {
  ...
  result <- try(uniroot(root_func, interval = x_interval, extendInt = "yes"), silent = TRUE)
...
find_all_x_for_y <- function(y_target, x_values, y_values, n_segments = 3) {
...
task2_df %>% add_predicted_draws(s.2.va.y) %>% 
  ...
  mutate(x_vals = rep(list(seq(-5, 5, 0.01)), n())) %>% # unnest(x_vals) %>% 
