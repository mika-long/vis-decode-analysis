...
phy.y_pred <- size(va.y_pred, 485)
pixel.y_pred <- phy_to_pixel_y(phy.y_pred, 3.73)
data.y_pred <- pixel_to_data_y(pixel.y_pred)

data.frame(y = y_pred) %>% 
  ggplot(aes(y = y_pred)) + stat_halfeye() + ylim(0.25, 0.3)

sapply(data.y_pred, function(x) find_all_x_for_y(x, x_vals, y_vals)) %>% 
  unlist(.) %>% as.data.frame(.) %>% 
  pivot_longer(cols = everything()) %>%
  ggplot(aes(x = value + 1.3)) + stat_halfeye()
