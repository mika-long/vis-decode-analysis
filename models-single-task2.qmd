...
# another option for single 
single_df <- read.csv("single_pid.csv") %>% as_tibble(.) %>% 
  filter(grepl("task", trialId) | grepl("test", trialId)) %>% 
  select(participantId, trialId, responseId, answer) %>% 
  filter(!responseId %in% c("dist-calibration-CM", "ball-positions")) %>% 
  mutate(answer = as.numeric(answer)) %>% 
  pivot_wider(names_from = responseId, values_from = answer, names_repair = "universal") %>% 
  separate_wider_delim(trialId, delim = "_", names = c("task", "type", "id")) %>% 
  select(-type) %>% 
...
participants <- read.csv("single_pid.csv") %>% as_tibble(.) %>% 
  filter(grepl("calibra", trialId)) %>% 
  select(participantId, responseId, trialId, answer) %>% 
  filter(grepl("MM", responseId)) %>% 
  ...
  mutate(pixelToMM = as.numeric(pixelToMM), 
         dist_to_screen = as.numeric(dist_to_screen))

# single_df %>% left_join(participants, by = join_by(participantId)) -> single_df
```
...
task2_df <- single_df %>% filter(task == "task2") %>% 
...
$$\lambda \sim \mathcal{N}(1.5, 1)$$

```{r}
f <- bf(error.y ~ 1, 
        family = exponential())

p <- prior(normal(1.5, 1), class = Intercept)
```
...
task2_df %>% mutate(error.y = va.ans.y - va.select.y) %>% 
  get_prior(f, data = ., prior = prior(normal(1.5, 1)))

task2_df %>% mutate(error.y = va.ans.y - va.select.y) %>% 
  mutate(ifelse(error.y < 0, 0, error.y)) %>% 
  # mutate(log.error.y = log(error.y * 10)) %>% 
...
s.2.va.y <- task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>% 
  brm(formula = f, 
      prior = prior(normal(1.5, 1), class = Intercept), 
      cores = 4, 
...
Hmm perhaps this is a gamma distribution rather than a exponential distribution? We know that the Gamma distribution is parameterized by two things: a shape parameter $\alpha$ and a rate parameter $\theta$^[See [stan](https://mc-stan.org/docs/functions-reference/positive_continuous_distributions.html#gamma-distribution) and [wiki](https://en.wikipedia.org/wiki/Gamma_distribution)]: 
...
```{r}
s.2.va.y.1 <- task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>% 
  # mutate(error.y = ifelse(error.y < 0, 0, error.y)) %>% 
  brm(f, 
      cores = 4, 
      data = ., 
      file = "models/s.2.va.y.1")
```
...
summary(s.2.va.y.1)
plot(s.2.va.y.1)
pp_check(s.2.va.y.1, ndraws = 100)
```

```{r}
loo(s.2.va.y, s.2.va.y.1)
```

So this Gamma distribution fits better than the exponential distribution. 

What if we model `log(error.y)` instead of `error.y` directly? 

```{r}
task2_df %>% mutate(error.y = log(abs(va.ans.y - va.select.y))) %>% 
  ggplot(aes(x = error.y)) + geom_dots()
```
