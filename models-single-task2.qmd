...
#| code-fold: true 
x <- seq(-5, 5, 0.01)
y <- dsgt(x, mu = -1.3, sigma = 1.3, lambda = 0.3, p = 2.9, q = 46.8, mean.cent = FALSE)
data.frame(x = x, y = y) %>% 
  ggplot(aes(x = x, y = y)) + 
  geom_line() + 
  xlim(-5, 5) + 
...
Create a function that uses `approxfunc` and `optimize` to find value of `x` given `y`: 

TODO: start from the following: 

```{r}
# Create an interpolating function from your discrete points
create_interp_func <- function(x_values, y_values) {
  interp_func <- approxfun(x_values, y_values, rule = 2)
  return(interp_func)
}
...
find_x_for_y_uniroot <- function(y_target, x_values, y_values, x_interval) {
  ...
  interp_func <- create_interp_func(x_values, y_values)
  ...
  result <- try(uniroot(root_func, interval = x_interval), silent = TRUE)
...
find_all_x_for_y <- function(y_target, x_values, y_values, n_segments = 10) {
  x_min <- min(x_values)
  x_max <- max(x_values)
...
Let's draw from the posterior distribution :

```{r}
posterior_predict(s.2.va.y) %>% as.vector(.) %>%
  sample(., 100) %>% 
  data.frame(.) %>% 
  ggplot(aes(y = -`.`)) + stat_halfeye()
```
