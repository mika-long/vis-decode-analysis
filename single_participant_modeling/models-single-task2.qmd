...
```{r}
#| output: false 
model.lomax <- task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>%
...
pp_check(model.lomax, ndraws = 100) # + xlim(0, 0.5) # hmm can't really tell which one is better ... 
```

```{r}
model.lomax %>% get_variables(.)

model.lomax %>% spread_draws(b_lambda_Intercept, b_alpha_Intercept) %>% 
...
task2_df %>% mutate(error.x = va.select.x - va.ans.x, 
                    error.x = ifelse(abs(error.x) < 1e-10, 0, error.x)) %>% 
  ggplot(aes(x = error.x)) + geom_dots() + 
...
Stan: https://mc-stan.org/docs/functions-reference/unbounded_continuous_distributions.html#cauchy-distribution
