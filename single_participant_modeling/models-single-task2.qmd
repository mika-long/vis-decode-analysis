...
model.y.weibull <- task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>%
  # mutate(error.y = ifelse(error.y < 1e-10, 1e-10, error.y)) %>% 
  brm(
    formula = f,
    data = .,
...
```{r}
model.y.weibull %>% get_variables(.)

# TODO 
# model.y.weibull %>% spread_draws(b_Intercept) %>% 
#   mutate(mu = exp(b_Intercept)) %>% 
#   ggplot(aes(x = 1/mu)) + stat_halfeye()
```
...
prediction %>% ggplot() + 
  ...
  geom_dots(data = task2_df, aes(x = abs(va.ans.y - va.select.y))) + 
  xlim(0, 0.05)
```
...
model.y.gamma <- task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>%
  # mutate(error.y = ifelse(error.y < 1e-10, 1e-10, error.y)) %>% 
  brm(
    formula = f,
    data = .,
...
plot(model.y.gamma)
pp_check(model.y.gamma, ndraws = 100)
pp_check(model.y.gamma, ndraws = 100, type = "pit_ecdf")
```

Plot posterior: 

```{r}
model.y.gamma %>% get_variables(.)
...
prediction <- task2_df %>% 
  mutate(error.y = abs(va.ans.y - va.select.y)) %>% 
  add_predicted_draws(model.y.gamma, ndraws = 100)
...
```{r}
#| output: false 
#| label: Y fit Log Normal  

model.y.lognormal <- task2_df %>% mutate(error.y = abs(va.ans.y - va.select.y)) %>%
  mutate(error.y = ifelse(error.y < 1e-10, 1e-10, error.y)) %>% 
  brm(
    formula = f,
    data = .,
...
```{r}
model.y.lognormal %>% get_variables(.)
...
```{r}
loo(model.y.lomax, model.y.exponential, model.y.gamma, model.y.lognormal, model.y.weibull) 
```
