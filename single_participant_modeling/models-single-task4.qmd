...
Lomax is Pareto Type II distirbution. We had similar models for task 2. 
...
f <- bf(
  error ~ 1,  # Intercept-only model for mu (scale parameter)
  shape ~ 1  # Intercept-only model for shape parameter
)

p <- c(
    # Prior for the scale parameter (on log scale)
    prior(constant(log(1e-10)), class = "Intercept"),
...
s.4.2 <- task4_df %>% mutate(error = abs(data_to_screen_slope(data.select.slope) - data_to_screen_slope(data.ans.slope)) + epsilon) %>%  brm(
  formula = f,
  data = .,
...
expose_functions(s.4.2, vectorize = TRUE)
...
pp_check(s.4.2, ndraws = 100) + xlim(0, 1)
```
...
```{r}
s.4.2 %>% spread_draws(b_shape_Intercept) %>% 
