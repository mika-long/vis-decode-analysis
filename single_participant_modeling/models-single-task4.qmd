...
```{r}
summary(model4.lognormal)
plot(model4.lognormal)
pp_check(model4.lognormal, ndraws = 100)
pp_check(model4.lognormal, ndraws = 100, type = "pit_ecdf")
```

This `pit_ecdf` seems quite alright with Weibull ... 

```{r}
model4.lognormal %>% get_variables() 

model4.lognormal %>% spread_draws(b_Intercept, b_sigma_Intercept) %>% 
  mutate(mu = exp(b_Intercept), sigma = exp(b_sigma_Intercept)) %>% 
...
prediction <- task4_df %>% 
  mutate(error = abs(data_to_screen_slope(data.ans.slope) - data_to_screen_slope(data.select.slope))) %>% 
  mutate(error = ifelse(abs(error) < tolerance, tolerance, error)) %>% 
  add_predicted_draws(model4.lognormal, ndraws = 100)
...
```{r}
summary(model4.gamma)
plot(model4.gamma)
pp_check(model4.gamma, ndraws = 100)
pp_check(model4.gamma, ndraws = 100, type = "pit_ecdf")
```

This `pit_ecdf` seems quite alright with Weibull ... 

```{r}
model4.gamma %>% get_variables() 

model4.gamma %>% spread_draws(b_Intercept, b_shape_Intercept) %>% 
  mutate(mu = exp(b_Intercept), shape = exp(b_shape_Intercept)) %>% 
  pivot_longer(c(mu, shape)) %>% 
  ggplot(aes(x = value, y = name)) + 
  stat_halfeye()
...
prediction <- task4_df %>% 
  mutate(error = abs(data_to_screen_slope(data.ans.slope) - data_to_screen_slope(data.select.slope))) %>% 
  mutate(error = ifelse(abs(error) < tolerance, tolerance, error)) %>% 
  add_predicted_draws(model4.gamma, ndraws = 100)
...
```{r}
loo(model4.exponential, model4.lomax, model4.weibull, model4.lognormal, model4.gamma)
```
