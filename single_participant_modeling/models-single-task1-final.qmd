...
summary <- fit$summary()
custom_pp_check(fit)
# custom_pp_check(fit, type = "pit_ecdf")
```
...
### Compare the two weighted average models 

```{r}
m1 <- readRDS("models/task1.weighted.rds")
m1_loo <- m1$loo()
m2 <- readRDS("models/task1.weighted_big.var.rds")
m2_loo <- m2$loo()
brms::loo_model_weights(list("Model 1" = m1_loo, "Model 2" = m2_loo),
                  method = c("stacking"))
```

The one with the bigger variance has slightly better performance. 
