...
p <- c(
  ...
  # prior(normal(0, 1), class = b, coef = "va.ans.y", dpar = sigma, lb = 0) # this is beta 
)
```
...
get_prior(formula = f,
          data = ., 
          # familiy = brmsfamily("gaussian", link_sigma = "identity")
          )

task5_df %>% mutate(error_va = va.select.x - va.ans.x, 
                    log.va.ans.y = log(va.ans.y)) %>%
  make_stancode(f, data = ., # family = brmsfamily(family = "gaussian", link_sigma = "identity"), 
                prior = p)
```
...
s.5x.va.1 <- task5_df %>% mutate(error_va = va.select.x - va.ans.x, 
  ...
  brm(
    ...
    prior = p,
    data = .,
    # family = brmsfamily(family = "gaussian", link_sigma = "identity"),
    chains = 4, 
...
**Option 2**: $\sigma_i = \exp(f(d_i)) = \exp(\alpha + \beta \times \texttt{va.ans.y}_i)$
...
Let's check the posterior of alpha: 

```{r}
#| code-fold: true 

s.5y.va.1 %>% spread_draws(b_sigma_Intercept) %>% 
  mutate(alpha = exp(b_sigma_Intercept)) %>% 
  ggplot(aes(x = alpha)) + 
  stat_halfeye() + 
  xlim(0.01, 0.02)
