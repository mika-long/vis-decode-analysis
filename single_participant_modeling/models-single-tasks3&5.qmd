...
task5.x.no.bias <- task5_df %>% mutate(error_va = va.select.x - va.ans.x, 
                                 log.va.ans.y = log(va.ans.y)) %>%
...
```{r}
summary(task5.x.no.bias)
```

```{r}
#| layout-ncol: 2
plot(task5.x.no.bias)
pp_check(task5.x.no.bias, ndraws = 100)
```

Check the posterior estimations of `alpha`: 

```{r}
task5.x.no.bias %>% spread_draws(`b_sigma_.*`, regex=TRUE) %>% 
...
prediction <- task5_df %>% mutate(error_va = va.select.x - va.ans.x, 
  ...
  add_predicted_draws(task5.x.no.bias, ndraws = 100)
...
```{r}
loo(task5.x.no.bias, task5.x.bias)
```
