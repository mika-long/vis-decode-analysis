...
task1_df %>% mutate(mix_paramter = as_draws_rvars(fit)$lambda) %>%
  unnest_rvars() %>%
  ggplot(aes(x = param.lambda, y = mix_paramter)) +
  ggdist::stat_lineribbon() + ylim(0, 1)
```
...
$$\begin{align*}
    ...
    \hat{x}^\text{mod}_i &\sim \text{Normal}(x^\text{mod}_i + \mu^\text{mod}, b^\text{mod}) \\ 
...
NOTE --- we are looking for small variance when lambda = 0. The above plot does not give us this fact. 
