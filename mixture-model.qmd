...
```{r}
model <- cmdstan_model("mix_models/mix.laplace.stan")
```
...
This has 23 out of 8000 divergent transitions. 

Let's check the fitted model: 

```{r}
summary <- fit$summary()

summary %>% filter(rhat > 1.01)

# plot posterior summaries 
summary %>% filter(variable %in% c("intercept_mu_mod", "var_mu_mod", 
                                   "intercept_mu_med", "var_mu_med")) %>% 
  ggplot(aes(y = variable)) + 
  geom_segment(aes(x = q5, xend = q95, yend = variable)) +  
  geom_point(aes(x = median)) # + 
  # xlim(0, 1)

summary %>% filter(variable %in% c("theta")) %>% 
  ggplot(aes(y = variable)) + 
  geom_segment(aes(x = q5, xend = q95, yend = variable)) +  
  geom_point(aes(x = median))
```
...
y_rep <- posterior_samples %>% 
  select(starts_with("hat_x_med"), starts_with("hat_x_mod")) %>% 
   as.matrix(.)

component_indicators <- posterior_samples %>%
  select(starts_with("component_indicator")) %>%
  as.matrix()
```

# Stan code --- Laplace distribution + participant-level theta 

Read in stan code: 

```{r}
model <- cmdstan_model("mix_models/mix.laplace.2.stan")
```
...
# save model 
fit$save_object(file = "mix_models/m5.rds")
```

This has 1% divergent transitions. Let's check the fitted models: 

```{r}
summary <- fit$summary()

# summary %>% filter(rhat > 1.001)
...
summary %>% filter(str_detect(variable, "^theta")) %>% 
  ggplot(aes(y = variable)) + 
  geom_segment(aes(x = q5, xend = q95, yend = variable)) +  
  geom_point(aes(x = median))
```

This just tell us on average participants overwhelmingly select the median operator, instead of the mode operator ... 

# Stan code --- Laplace distribution + lambda varies with |med - mod| 

The generative model for `theta` is defined as 
$$\text{logit}(\theta_i) = \beta_0 + \beta_{\text{dist}} \cdot |x^{med}_i - x^{mod}_i| + z^\theta_{PID[i]} \cdot \sqrt{\text{var_theta}}$$


Read in stan code: 

```{r}
model <- cmdstan_model("mix_models/mix.laplace.3.stan")
```
...
# save model 
fit$save_object(file = "mix_models/m6.rds")
```

- if we don't specify anything about the variance or mean order between med and mod, than we get more divergent transitions ... around 104 
- if we do specify variance (var_med > var_mod), then: 
  - 58 of 120000 has divergent transitions (unsigned)
  - 54 of 120000 has divergent transitions (signed)
- if we specify mean and no variance: 
  - 139 of 12000 ended with a divergence 
- if we specify mean AND variance: 
  - 40 of 12000 transitions ended with a divergence  


Let's look at the fitted results: 

```{r}
summary <- fit$summary()

summary %>% filter(str_detect(variable, "^mu_")) %>% 
  ggplot(aes(y = variable)) + 
  geom_segment(aes(x = q5, xend = q95, yend = variable)) +  
  geom_point(aes(x = median))
```

# Fit single participant with stan 

## Assume that `lambda ~ beta(2, 2)` + single participant 
...
# save model 
fit$save_object(file = "mix_models/sheng.m1.rds")
```
...
summary %>% filter(str_detect(variable, "lambda")) %>% 
  ...
  geom_segment(aes(x = q5, xend = q95, yend = variable)) +  
  geom_point(aes(x = median))
# with almost certainty I am choosing the median operator lol 

summary %>% filter(str_detect(variable, "^mu")) %>% 
  ggplot(aes(y = variable)) + 
  geom_segment(aes(x = q5, xend = q95, yend = variable)) +  
  geom_point(aes(x = median))

summary %>% filter(str_detect(variable, "^sigma")) %>% 
  ggplot(aes(y = variable)) + 
  geom_segment(aes(x = q5, xend = q95, yend = variable)) +  
  geom_point(aes(x = median))
```
...
bayesplot::ppc_dens_overlay(stan_data$x, y_rep[1:100, ])
```

This fit actually doesn't look that bad. 

## Modeling the effect of median and mode 

Read in stan code: 

```{r}
model <- cmdstan_model("mix_models/sheng.mix.2.stan")
```
...
# save model 
fit$save_object(file = "mix_models/sheng.m2.3.rds")
```
...
summary %>% filter(str_detect(variable, "beta")) %>% 
  ggplot(aes(y = variable)) + 
  geom_segment(aes(x = q5, xend = q95, yend = variable)) +  
  geom_point(aes(x = median))
...
The difference between using `abs(diff)` and `diff^2` isn't that big ... but the difference between `diff` and `abs(diff)` is definitely there ... 

I think we would need a `loo` comparison ... 

```{r}
m1 <- readRDS("mix_models/sheng.m2.1.rds") # diff 
m1_loo <- m1$loo()
m2 <- readRDS("mix_models/sheng.m2.2.rds") # abs diff 
m2_loo <- m2$loo()
m3 <- readRDS("mix_models/sheng.m2.3.rds") # diff^2
m3_loo <- m3$loo()
...
From this we know the model using absolute difference between median and mode is the best one^[we can verify this using `m2$code()` to check the original code used to run]. 
...
```{r}
y_rep <- m2$draws("y_rep", format = "matrix")
