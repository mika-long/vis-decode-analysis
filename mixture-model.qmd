...
```{r}
task1_df %>% ggplot(aes(x = abs(va.med.x - va.mod.x))) + geom_dots() -> p
p
```
...
We will compare two variations of this --- one where we model a non-zero intercept for `hat(x)^med` and `hat(x)^mod`, the other where we model a zero intercept of the two. 

The following model as a non-zero intercept. 

```{r}
#| eval: false 
model <- cmdstan_model("mix_models/sheng.mix.2.2.1.stan")
# model
``` 
...
#| echo: false
#| output: false 
fit <- readRDS("mix_models/sheng.mix.2.2.1.rds")
```
...
fit$save_object(file = "mix_models/sheng.mix.2.2.1.rds")
```

---

The following model as zero intercept. 

```{r}
#| eval: false 
model <- cmdstan_model("mix_models/sheng.mix.2.2.2.stan")
# model
``` 
...
#| echo: false
#| output: false 
fit <- readRDS("mix_models/sheng.mix.2.2.2.rds")
```
...
```{r}
# save the model 
fit$save_object(file = "mix_models/sheng.mix.2.2.2.rds")
```
...
```{r}
m1 <- readRDS("mix_models/sheng.mix.2.2.1.rds") # non-zero intercept 
m1_loo <- m1$loo()
m2 <- readRDS("mix_models/sheng.mix.2.2.2.rds") # zero-intercept 
m2_loo <- m2$loo()
...
It appears that the model with the non-zero intercept is better. 
...
m1_loo <- m1$loo()
m2 <- readRDS("mix_models/sheng.mix.2.2.1.rds") # abs diff
m2_loo <- m2$loo()
...
The second model (absolute difference) seems to be fitting well. 
...
\beta &\sim \mathcal{N}(0, 1), \alpha \sim ...
\end{align*}$$
...
## lambda = inv-logit(beta * (abs(x.med - x.mod) / var - alpha))
...
task1_df %>% mutate(y_rep = as_draws_rvars(fit)$y_rep - va.med.x) %>% 
  ...
  geom_point(aes(y = va.select.x - va.med.x), data = task1_df)
```
...
```{r}
#| eval: false 
model <- cmdstan_model("stan_files//sheng.mix.2.7.stan")
``` 
...
#| echo: false
#| output: false 
fit <- readRDS("mix_models/sheng.mix.2.7.rds")
```
...
fit$save_object(file = "mix_models/sheng.mix.2.7.rds")
```
...
Let's see what the lambda values look like for `m5`: 
...
m5 <- readRDS("mix_models/sheng.mix.2.5.rds") # abs diff
summary <- m5$summary()
...
task1_df %>% mutate(mix_paramter = as_draws_rvars(fit)$lambda) %>% 
  unnest_rvars() %>% 
  ggplot(aes(x = param.lambda, y = mix_paramter)) + 
  stat_ribbon() + ylim(0, 1)

m5$code()
```

# What if we don't model `mu_med` and `mu_mod` ...? 
