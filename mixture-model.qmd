...
\hat{y}^{\text{mod}}_i &\sim \text{Laplace}(x^\text{mod}, (\sigma^{\text{mod}})^2) \\ 
(1) \,\, \lambda &\sim \text{Beta}(2, 2) 
\end{align*}$$

## Same lambda 

If we set $\lambda \sim \text{Beta}(5, 5)$ then the sampling is more concentrated around 0.5, and if we set $\lambda \sim \text{Beta}(2, 2)$ then the sampling is closr to a uniform distribution over [0, 1]. 
...
### lambda = inv-logit(k * (med - mod))

... 

### lambda = inv-logit(k * abs(med - mod))

...

### lambda = inv-logit(k * (med - mod)^2)
