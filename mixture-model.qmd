...
```{r}
task1_df %>% ggplot(aes(x = abs(va.med.x - va.mod.x))) + geom_dots()
```
...
custom_pp_check(fit, transform = \(x) x - task1_df$va.med.x)
```
...
Note that we will be placing a negative prior on alpha. 
...
$$\begin{align*}
\hat{x}_i & =  w_i \cdot \hat{x}^{\text{med}}_i + (1 - w_i) \cdot \hat{x}^{\text{mod}}_i \\
\hat{x}^{\text{med}}_i &\sim \mathcal{N}(x^\text{med}_i, (\sigma^\text{med})^2) \\ 
\hat{x}^{\text{mod}}_i &\sim \mathcal{N}(x^\text{mod}_i, (\sigma^{\text{mod}})^2) \\ 
w_i &= \frac{\frac{1}{\operatorname{MSE}(\hat{x}^{\text{med}}_i)}}{\frac{1}{\operatorname{MSE}(\hat{x}^{\text{med}}_i)} + \frac{1}{\operatorname{MSE}(\hat{x}^{\text{mod}}_i)}}\\
&= \frac{\frac{1}{(\sigma^\text{med})^2}}{\frac{1}{(\sigma^\text{med})^2} + \frac{1}{(x^{\text{med}}_i - x^{\text{mod}}_i)^2 + (\sigma^\text{mod})^2}}
\end{align*}$$
...
```{r}
#| eval: false 
model <- cmdstan_model("mix_models/sheng.mix.2.5_scaled_weighted_imse.stan")
``` 
...
fit %>% 
  gather_draws(`mu_med`, `mu_mod`) %>% 
  ggplot(aes(x = .value, y = .variable)) + 
  stat_halfeye()

fit %>% 
  gather_draws(`sigma_med`, `sigma_mod`) %>% 
  ggplot(aes(x = .value, y = .variable)) + 
  stat_halfeye() +
  expand_limits(x = 0)
...
task1_df %>% mutate(mix_paramter = as_draws_rvars(fit)$lambda) %>% 
  unnest_rvars() %>% 
  ggplot(aes(x = va.med.x - va.mod.x, y = mix_paramter)) + 
...
task1_df %>% 
  mutate(
    inverse_mse_med = as_draws_rvars(fit)$inverse_mse_med, 
    inverse_mse_mod = as_draws_rvars(fit)$inverse_mse_mod
  ) |>
  pivot_longer(starts_with("inverse_mse_")) |>
  unnest_rvars() |>
  ggplot(aes(x = va.med.x - va.mod.x)) + 
  stat_ribbon(aes(y = sqrt(1/value), fill = name, fill_ramp = after_stat(level))) 
```


```{r}
task1_df %>% mutate(pred = as_draws_rvars(fit)$y_rep) %>% 
  unnest_rvars() %>% 
  ggplot(aes(x = va.med.x - va.mod.x, y = pred - va.med.x)) + 
  stat_lineribbon() +
