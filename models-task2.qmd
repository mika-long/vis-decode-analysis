...
task2_df <- # task_df %>% filter(task == "task2") %>% 
  single_pid_df %>% filter(task == "task2") %>% 
...
task2_df %>% mutate(error_va = va.ans.x - va.select.x) %>% 
  make_stancode(f, data = ., family = c_family, prior = p,
...
m1.1.2 <- task2_df %>% mutate(error_va = va.ans.x - va.select.x) %>%
  brm(
    formula = f, 
    family = c_family, 
    stanvars = stanvar(scode = laplace_lpdf, block = "functions"), 
