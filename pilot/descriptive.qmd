...
# maybe also 6679e88eb948e8a4b08a39fb 

task_df %>% filter(!participantId %in% exclude_participants) -> task_df
saveRDS(task_df, file = "task.rds")
```
...
participants %>% filter(dist_to_screen < 250)

participants %>% filter(!participantId %in% exclude_participants) %>% 
  saveRDS(., "participants.rds")
```
...
```{r}
task1_df %>% filter(participantId == "6679e88eb948e8a4b08a39fb") %>% 
  ggplot(aes(x = data.select.x, y = data.select.y)) + 
  geom_point() + xlim(-5, 5) + ylim(0, 1) + 
  geom_point(data = filter(task1_df, participantId == "6679e88eb948e8a4b08a39fb"), aes(x = data.ans.x, color = "answer"))
```

The answer is good but the distance to screen is highly unreasonable ... 
