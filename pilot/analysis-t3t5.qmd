...
library(ggdist)
# setting the theme 
theme_set(theme_minimal() + theme(text = element_text(size = 8)))
# model
library(brms)
# plot results 
library(tidybayes)
...
```{r}
f <- bf(error_va ~ 0,  # could also be error_va ~ 1 or error_va ~ (1 | participantId)
...
task5_df %>% mutate(error_va = va.select.x - va.ans.x) %>%
  ...
  mutate(error_va = 0) %>% 
  add_epred_draws(m.5.va, dpar = "sigma", re_formula = NA)

prediction_grid <- task5_df %>% data_grid(va.ans.y = seq_range(va.ans.y, n = 50)) %>% 
  add_predicted_draws(m.5.va, re_formula = NA, ndraws = 100)
...
ggplot(prediction_grid, aes(x = va.ans.y, y = .prediction)) + 
  ...
  geom_point(data = task5_df, aes(y = va.select.x - va.ans.x, x = va.ans.y), alpha = 0.8) +
...
# save 
ggsave("task5-project-x.svg", p1, height = 5, width = 10, units = "cm", dpi = 300)
...
ggplot(prediction_grid, aes(x = va.ans.x, y = .prediction)) + 
  ...
  geom_point(data = task5_df, aes(y = va.select.y - va.ans.y, x = va.ans.x), alpha = 0.8) + 
  ylim(-0.5, 0.5) + 
...
ggsave("task5-project-y.svg", p2, height = 5, width = 10, units = "cm", dpi = 300)
```
...
ggsave("task5-combined.svg", combined, height = 8, width = 10, units = "cm", dpi = 300)
```
...
prediction_grid <- task3_df %>% data_grid(va.ans.x = seq_range(va.ans.x, n = 50)) %>% 
...
ggplot(prediction_grid, aes(x = va.ans.x, y = .prediction)) + 
  ...
  geom_point(data = task3_df, aes(y = va.ans.y - va.select.y, x = va.ans.x), alpha = 0.8) +
  labs(title = "Task 3", y = "Signed error", x = "Distance to Y-Axis") -> p3
...
```{r}
ggsave("t3t5-combined.svg", plot = combined, height = 10, width = 10, units = "cm")
```
