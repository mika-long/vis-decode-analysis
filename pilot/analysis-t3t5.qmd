...
pp_check(m.5.x.va, ndraws = 100, type = "pit_ecdf") + 
  labs(title = "Task 5 (X)") + 
  theme(text = element_text(size = 12)) -> task5.x.ecdf

ggsave("plots/task5-x-pit-ecdf.pdf", task5.x.ecdf, width = 10, height = 8, unit = "cm", dpi = 300)
```
...
ggplot(prediction_grid, aes(x = va.ans.y, y = .prediction)) + 
  ...
  geom_point(data = filter(task5_df, !participantId %in% task5.x.exclude), aes(y = va.select.x - va.ans.x, x = va.ans.y), 
             alpha = 0.5, 
             size = 0.5) +
  labs(title = "Project from dot to x-axis)", y = "Signed Error", x = "Distance to X-Axis") + 
...
pp_check(m3, ndraws = 100, type = "pit_ecdf") + 
  theme_minimal() + 
  labs(title = "Task 3") + 
  theme(text = element_text(size = 12)) -> task3_pit  

ggsave("plots/task3_pit.pdf", task3_pit, height = 8, width = 10, unit = "cm", dpi = 300)
```
