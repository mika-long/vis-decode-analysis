...
ESS is still too low ... this might not be a good one ... 
...
model.y.weibull <- task2_df %>% mutate(error.y = va.ans.y - va.select.y) %>% 
  mutate(error.y = ifelse(error.y < tolerance, tolerance, error.y)) %>% # brms needs value to be > 0  :Error: Family 'weibull' requires response greater than 0.
  brm(formula = f, 
      data = ., 
      prior = p, 
      chains = 4, 
...
pp_check(model.y.weibull, ndraws = 100, type = "pit_ecdf") + 
  labs(title = "Task 2") + 
  theme(text = element_text(size = 12)) -> task3.y

ggsave("plots/task2.y.pdf", plot = task3.y, height = 8, width = 10, units = "cm", dpi = 300)
```
...
task2_df %>% modelr::data_grid(error.y = 0, 
  ...
  stat_slab(aes(x = .prediction, group = .draw), 
               color = "lightblue", alpha = 0.5, fill = NA, n = 3000) + 
  scale_thickness_shared() + 
  coord_cartesian(xlim = c(0, 0.1)) + 
  geom_rug(data = task2_df, aes(x = va.ans.y - va.select.y)) + 
  stat_slab(data = task2_df, aes(x = va.ans.y - va.select.y), 
            color = "darkblue", fill = NA, n = 3000) + 
  labs(y = "density", x = "signed error" ) + 
  scale_y_continuous(breaks = 0) -> p
  
p

ggsave("models/task2-y.pdf", plot = p, height = 5, width = 5, units = "cm")
```
...
ggsave("plots/task2.y.weibull.pdf", plot = p, height = 5, width = 5, unit = "cm", dpi = 300)
```
...
model.y.lognormal <- task2_df %>% 
  ...
  mutate(error.y = ifelse(error.y < tolerance, tolerance, error.y)) %>% # Error: Family 'lognormal' requires response greater than 0.
  brm(formula = f, 
      data = ., 
      prior = p, 
      chains = 4, 
...
A lot of divergent transitions ... 
...
ggsave("plots/task2.x.gaussian.pdf", task2.x.gaussian, height = 5, width = 8, unit = "cm", dpi = 300)

pp_check(model.x.normal, ndraws= 100, type = "pit_ecdf") + 
  theme(text = element_text(size = 10)) + 
  labs(title = "Task 2") -> task2.x.gaussian.pit

ggsave("plots/task2.x.gaussian-pit.pdf", task2.x.gaussian.pit, height = 4, width = 8, unit = "cm", dpi = 300)
...
ggsave("plots/task2.y.laplace.pdf", task2.laplace, height = 4, width = 8, unit = "cm")

pp_check(model.x.laplace, ndraws = 100, type = "pit_ecdf") + 
  theme(text = element_text(size = 10)) + 
  labs(title = "Task 2 -- Error on Y -- Laplace") -> task2.laplace.pit

ggsave("plots/task2.y.laplace-pit.pdf", task2.laplace.pit, height = 4, width = 8, unit = "cm")
...
task2_df %>% # mutate(error.x = va.select.x - va.ans.x) %>%
  ...
  group_by(.draw) %>% 
  ggplot() + 
  geom_density(aes(x = .prediction, group = .draw), color = "skyblue", 
               alpha = 0.2, linewidth = 0.5) + 
  # stat_slab(aes(x = .prediction), 
  #           fill = "skyblue", alpha = 0.6) + 
  geom_density(data = task2_df, aes(x = va.select.x - va.ans.x)) + 
  geom_rug(data = task2_df, aes(x = va.select.x - va.ans.x)) + 
  labs(y = "Probability", x = "Signed error (on X)") + 
  theme_minimal(base_size = 8) # -> p 
