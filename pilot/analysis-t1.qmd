...
# go and delete the installed cmdstanr ... 
...
J <- length(unique(task1_df$participant_id))
...
stan_data <- list(
  ...
  mu_pop_mean = 0.01,            # Mean of participant intercepts
  mu_pop_sd = 0.06,              # SD of participant intercepts
  log_sigma_pop_mean = -1.30,    # Mean of log(sigma) - KEEPING ON LOG SCALE
  log_sigma_pop_sd = 0.20    
)
```
