...
model.exponential <- task4_df %>% 
  ...
  mutate(error = ifelse(abs(error) < tolerance, tolerance, error)) %>% # exponent need rsp > 0 
  brm(formula = f, 
      data = ., 
      prior = p, 
      chains = 4, 
...
task4_df %>%  mutate(error = abs(data_to_screen_slope(data.select.slope) - data_to_screen_slope(data.ans.slope))) %>%
  mutate(error = ifelse(error < 1e-10, 0, error)) %>% 
...
model.weibull <- task4_df %>% 
  mutate(error = abs(data_to_screen_slope(data.ans.slope) - data_to_screen_slope(data.select.slope))) %>% 
  mutate(error = ifelse(abs(error) < tolerance, tolerance, error)) %>% # how come weibull also requires resposne > 0 ...? 
  brm(formula = f, 
      data = ., 
      prior = p, 
      chains = 4, 
...
Here's the thing --- I think with the type II pareto distribution, it assigns too much of a heavy tail to errors ... 
