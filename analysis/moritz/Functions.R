...
bestbf <- function(BF, digits = 2, restrict = NA, save = FALSE) {
  ...
  if (max(as.vector(BF)) < 1) {
    ...
    if (save) {
      ...
      if (is.na(restrict)) {
        outs$models <- round(out, digits = digits)
      } else {
    ...
    out <- sort(as.vector(BF[which.max(BF)]) / as.vector(BF), decreasing = F)
    if (save) {
...
bma <- function(bf) {
  ...
  terms <- terms[order(unlist(lapply(
...
wsAnova = function(dat, SStype = 3, spss = F) {
  has.car = require(car)
  if (!has.car) {
    return("No \"car\"package, no ANOVA\n")
  }
  ...
  facnr = facn[nifac:1]
  e.mv = matrix(
    unlist(tapply(dat[, dvnam], dat[, facnr], function(x) {
      x
    })),
    ncol = length(ifacn),
    dimnames = list(snams, ifacn)
  )
  print(summary(
    Anova(
      lm(e.mv ~ 1),
...
add.bars = function(
  ...
  plotbars <- function(x, m, l, h, len, col = "black") {
    for (j in 1:length(x)) {
      arrows(x[j], m[j], x[j], l[j], length = len, angle = 90, col = col)
    }
    for (j in 1:length(x)) {
      arrows(x[j], m[j], x[j], h[j], length = len, angle = 90, col = col)
    }
  }

  if (any(is.na(xvals))) {
    if (is.matrix(mn)) {
      xvals <- as.numeric(dimnames(mn)[[2]])
    } else {
  ...
  if (!is.matrix(mn)) {
    plotbars(xvals, mn, lo, hi, col = col, len = len)
  } else {
    for (i in 1:dim(mn)[1]) {
      plotbars(
        x = xvals,
...
mixedAnova = function(
  ...
  if (length(wsfacn) > 0) {
    dat <- dat[do.call(order, dat[, c(sfac, wsfacn)]), ]
  }
  for (i in 1:(dim(dat)[2] - 1)) {
    dat[, i] <- factor(dat[, i])
  }
  snams = levels(dat[, sfac])
  ns = length(snams)
  nifac = length(wsfacn)
  lev1s = unlist(lapply(lapply(dat[, wsfacn], levels), function(x) {
    x[1]
  }))
  bsfacs = dat[
    apply(dat[, wsfacn, drop = F], 1, function(x) {
      all(x == lev1s)
    }),
    bsfacn,
    drop = F
  ...
  if (nifac > 0) {
    ...
    e.mv = matrix(
      unlist(
        tapply(dat[, dvnam], dat[, wsfacn[length(wsfacn):1]], function(x) {
          x
        })
      ),
      ncol = length(ifacn),
    ...
    print(summary(
      Anova(
        ...
        idata = idata,
        type = SStype,
        idesign = formula(paste("~", paste(wsfacn, collapse = "*")))
      ),
      multivariate = F
    ))
  } else {
...
summarySEwithin <- function(
  ...
  factorvars <- vapply(
    data[, c(betweenvars, withinvars), drop = FALSE],
    FUN = is.factor,
    FUN.VALUE = logical(1)
  )
