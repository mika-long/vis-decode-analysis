...
# task2_df %>%
#   ggplot(aes(y = phy.ans.y - phy.select.y, x = dist_to_screen)) +
#   geom_point()
# task2_df %>% ggplot(aes(x = phy.ans.y - phy.select.y)) + geom_dots()
# task2_df %>%
#   ggplot(aes(x = (phy.ans.y - phy.select.y) / dist_to_screen)) +
#   geom_dots()
```
...
task2_df %>% ggplot(aes(x = va.select.x - va.ans.x)) + geom_dots()

task2_df %>%
  ggplot(aes(x = (phy.ans.x - phy.select.x) / dist_to_screen)) +
  geom_dots()
...
task2_df %>%
  ggplot(aes(y = va.select.x - va.ans.x, x = dist_to_screen)) +
  geom_point(alpha = 0.5)

task2_df %>%
  ggplot(aes(y = va.select.x - va.ans.x, x = param.lambda)) +
  geom_point(alpha = 0.5) +
...
task2_df %>% mutate(error.y = va.ans.y - va.select.y) %>% get_prior(f, data = .)

# generate stan model code
task2_df %>%
  mutate(error.y = abs(va.ans.y - va.select.y)) %>%
...
Observe that the we still have three divergent transitions, even though we have taken the number of iterations to 10000^[for context other models took numb of iterations about 4000]. This suggests that the lomax model is likely not the right model. 
...
task2_df %>%
  ...
  geom_rug(data = task2_df, aes(x = va.ans.y - va.select.y), linewidth = 0.5) +
  stat_slab(
    data = task2_df,
    aes(x = va.ans.y - va.select.y),
    color = "darkblue",
    fill = NA,
    n = 3000,
    size = 0.5
...
Note that the above have a lot of divergent transitions, even though we took `adapt_delta = 0.99`, suggesting that this is not the right model. We can also check the PIC-ECDF plot to confirm:  
