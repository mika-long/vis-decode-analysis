...
y_draws <- newdata |>
  ...
  mutate(
    num.prediction = va_to_numerical_y(.prediction, pixelToMM, distToScreen)
  )
...
x_draws <- newdata |>
  ...
  mutate(
    num.prediction = va_to_numerical_x(.prediction, pixelToMM, distToScreen)
  )

y_val <- dsgt(-0.3, mu = -1.1, sigma = 0.9, lambda = -0.3, p = 4.6, q = 3.17)

x_graph <- x_draws %>%
  ggplot(aes(x = .prediction - 0.3)) +
  stat_slab() + # + scale_y_reverse()
  coord_cartesian(xlim = c(-5, 5)) +
  labs(x = "", y = "")
y_graph <- y_draws %>%
  ggplot(aes(y = -num.prediction)) +
  stat_slab() +
