...
task2_df <- task_df %>%
  ...
  mutate(
    pixel.ans.x = data_to_pixel_x(data.ans.x),
...
task2_df %>%
  mutate(error_va = va.select.x - va.ans.x) %>%
...
task2_df %>%
  ggplot(aes(y = va.ans.y - va.select.y, x = dist_to_screen)) +
  geom_point()
...
# Plot a new example as inset graphs -- Max Slope

```{r}
library(sgt)
x <- seq(-5, 5, length.out = 100)
# cdf 
y <- psgt(x, mu = 0.1, sigma = 1, lambda = 0.2, p = 2.6, q = 2.17, mean.cent = TRUE)
...
# Plot a new example as inset graphs -- Bisect Area

```{r}
library(sgt)
x <- seq(-5, 5, length.out = 100)
# pdf
y <- dsgt(x, mu = -0.1, sigma = 1.6, lambda = 0.3, p = 2.6, q = 2.17, mean.cent = TRUE)
