...
p <- c(
  ...
  prior(normal(0, 1), class = Intercept, dpar = "sigma"), # hmm ...? 
  prior(normal(0, 1), class = sd, group = participantId, dpar = sigma, lb = 0) # hmm ...? 
)
```
...
ggsave(here("figs", "task5-x-pit-ecdf-diff.pdf"), p3, 
width = 10, height = 5, units = "cm", dpi = 300)
```
...
```{r}
f <- bf(error_va ~ 0 + (1 | participantId), # NEW CHANGE! 
