...
**Option 1**: $\log(\sigma_i) = f(d_i) \to \sigma_i = \exp(f(d_i)) = \exp(\alpha) \times d_i$
...
```{r}
s.3.va.1 %>% spread_draws(b_sigma_Intercept) %>% 
  mutate(alpha = exp(b_sigma_Intercept)) %>% 
  ggplot(aes(x = alpha)) + 
  stat_halfeye() + 
  xlim(0.01, 0.02)
...
**Option 3**: $\sigma_i = \alpha_i + d_i$ 

```{r}
f <- bf(error_va ~ 0,
        sigma ~ 1 + offset(va.ans.x), 
        family = brmsfamily("gaussian", link_sigma = "identity"))

p <- c(
  prior(normal(1, 1), class = Intercept, dpar = sigma, lb=0) # , 
  # prior(normal(1, 1), class = b, dpar = sigma, lb = 0)
)
```
...
plot(s.3.va.3)
pp_check(s.3.va.3, ndraws = 100)
```

```{r}
s.3.va.3 %>% get_variables(.)
```
...
```{r}
loo(s.3.va.1, s.3.va.2, s.3.va.3)
```

Ok from this we see that this relationship is definitely NOT additive. 
