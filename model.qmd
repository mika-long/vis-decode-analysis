...
:::{.caution-note}
We could also model $\sigma[i] = \texttt{va.ans.y} \times \hat{\sigma}[i] \equiv \log(\sigma[i]) = \log(\texttt{va.ans.y}) + \log(\hat{\sigma}[i])$ that would just imply we chagne the formula from `sigma ~ 1 + offset(log(va.ans.y)) + (1 | participantId) ` to `sigma ~ 0 + offset(log(va.ans.y)) + (1 | participantId)`.
:::
...
```{r}
m5.2.x %>% spread_draws(r_participantId__sigma[participantId, term]) %>% 
  filter(term == "Intercept") %>% 
  mutate(hat_sigma = exp(r_participantId__sigma)) %>% 
  ggplot(aes(y = participantId, x = hat_sigma)) + 
  # stat_pointinterval() 
...
m5.2.x <- add_criterion(m5.2.x, "loo")

loo_compare(m5.1.x, m5.2.x)
```
...
```{r}
f <- bf(error_phy ~ 0, 
        sigma ~ 0 + offset(log(phy.ans.x)) + offset(log(dist_to_screen)) + (1 | participantId))
p <- prior(constant(1), class = "sd", group = "participantId", dpar="sigma")
```
...
Huh things here are much nicer ... 

### Compare the two models 

```{r}
m5.1.y <- add_criterion(m5.1.y, "loo")
m5.2.y <- add_criterion(m5.2.y, "loo")
...
Huh this variance is much bigger than the one before ...? 
