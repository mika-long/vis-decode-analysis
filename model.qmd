...
$$
\begin{aligned}
\text{error_va}_{i} &\sim \mathcal{N}(\mu_{\text{PID}[i]}, \sigma_{\text{PID}[i]}) \\ 
\mu_j &\sim \mathcal{N}(0, 1), j \in [13]\\ 
\sigma_j &= k \cdot \texttt{va.ans.y} \cdot \hat{\sigma}_j \\ 
\sigma_j &\sim \text{Log-Normal}(\hat{\mu}, \hat{\sigma})
\end{aligned}
$$

$$
\begin{align}
\text{error_va}_{i} &\sim \mathcal{N}(\mu_{\text{PID}[i]}, \sigma_i) \\ 
\mu_j &\sim \mathcal{N}(\mu_0, \tau_\mu), j \in [13]\\ 
\sigma_i &= k \cdot \texttt{va.ans.y}_i \cdot \hat{\sigma}_{\text{PID}[i]} \\ 
\log(\hat{\sigma}_j) &\sim \mathcal{N}(\hat{\mu}, \hat{\tau})
\end{align}
$$

$$
\begin{align}
\text{error_va}_i &\sim N(\mu_{\text{PID}[i]}, \sigma_{\text{PID}[i]}) \\ 
\mu_j &\sim N(\mu_0, \tau_0) \\ 
\log(\sigma_{\text{PID}[i]}) &= \beta \cdot \log(\texttt{va.ans.y}_i) + \gamma_j
\end{align}
$$
...
\mu_j &\sim \mathcal{N}(\mu_0, \tau_\mu), j \in [13]\\
\log(\sigma_{j,i}) &= \beta \cdot \log(\texttt{va.ans.y}_i) + \gamma_j \\
\gamma_j &\sim \mathcal{N}(\gamma_0, \tau_\gamma)
$$
...
f <- bf(error_va ~ 1 + (1 | participantId), 
        ...
        sigma ~ log(va.ans.y) + (1 | participantId)
        )
...
f3 <- bf(error_phy ~ 1 + (1 | participantId), 
