...
As can be seen here, there is a clear difference in trend ... As skewness $\lambda$ changes, the difference between the median `data.ans.x` and the mode `param.mu` increases. Therefore, if we are using the mode as a "proxy", the adjustment is not quite there yet ... 
...
task1_df %>% ggplot(aes(y = (data.select.left_area / (1 - data.select.left_area))^1, x = param.lambda)) + 
  geom_point(alpha = 0.5) + 
  geom_point(aes(y = (data.select.left_area / (1 - data.select.left_area))^0.7, x = param.lambda), color = "red", alpha = 0.5) + 
...
- According to Steven's law, we have that $S = I^{0.7}$, or, perceived sensation = stimuli intensity `^ 0.7`. How do we go from this to that though? 
...
m2.2 <- task2_df %>% 
  ...
  mutate(error_phy = ifelse(abs(error_phy) < 1e-11, 0, error_phy)) %>% 
  brm(formula = f,
      data = ., 
      prior = p,
      control = list(adapt_delta = 0.99), # note this is higher than 0.95
