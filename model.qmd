...
Looking at the above results, `t1` is clearly better. Let's see what does the result say: 

```{r}
# Extract the posterior samples for the sigma coefficients
sigma_coefs <- m1 %>%
  spread_draws(b_sigma_logva.ans.y)

# Summary statistics for the coefficient
sigma_summary <- sigma_coefs %>%
  summarise(
    mean = mean(b_sigma_logva.ans.y),
    median = median(b_sigma_logva.ans.y),
    lower_95 = quantile(b_sigma_logva.ans.y, 0.025),
    upper_95 = quantile(b_sigma_logva.ans.y, 0.975)
  )
...
```{r}
# Plot the posterior distribution 
ggplot(sigma_coefs, aes(x = b_sigma_logva.ans.y)) +
  geom_density(fill = "skyblue", alpha = 0.5) +
  geom_vline(xintercept = 1, color = "red", linetype = "dashed") +
  labs(
    title = "Posterior Distribution of log(va.ans.y) Coefficient",
    subtitle = "Red dashed line indicates theoretical value of 1",
    x = "Coefficient Value",
    y = "Density"
  ) +
  theme_minimal()
...
The previous results were obtained using the default priors, which are weakly informative. What if we use a moderately informative prior that centers beta around 1? 
...
t3 <- task5_df %>% mutate(error_va = va.select.x - va.ans.x) %>%
  ...
  brm(
    ...
    data = .,
    family = gaussian(),
    prior = prior(normal(1, 0.5), class = "b", coef = "logva.ans.y", dpar = "sigma"),
    control = list(adapt_delta = 0.99),
    chains = 4, 
    # cores = 4,
    iter = 6000, 
...
t3 <- add_criterion(t3, criterion = "loo",  moment_match = TRUE)
loo_compare(m1, m2, t3)
```
Fascinating, so with a prior set to be close to 1 we get results much closer to `m1`. Let's plot the posterior: 
...
# in a nl format 
# f3 <- bf(error_phy ~ dist_to_screen * eta, 
...
f4 <- bf(error_phy.adj ~ 1 + (1 | participantId), 
         sigma ~ 1 + log(phy.ans.y) + (1 | participantId)
         )
```

Let's test and see which among f3 and f4 is better using default priors first: 
...
t1 <- add_criterion(t1, criterion = "loo")
t2 <- add_criterion(t2, criterion = "loo")
loo_compare(t1, t2)
```
