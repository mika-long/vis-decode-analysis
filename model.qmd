...
#| output: false 
#| code-fold: true
#| label: verify that the results are correct in scale
...
$$
:::{.callout-caution}
Another model would be to model the variance being influenced by the distance the participant is away from the screen. However, that would mean we would need **a theoretical justfication for why distance impacts error variance beyond its effect on visual angle**. 
:::
...
m5.x <- task5_df %>% mutate(error = va.select.x - va.ans.x) %>%
  brm(
...
m5.y <- task5_df %>%
  mutate(error = va.select.y - va.ans.y) %>%
  brm(
    formula = error ~ 1 + (1|participantId),
    data = .,
    family = gaussian(),
...
task1_df <- task_df %>% filter(task == "task1") %>% 
  ...
  mutate(left_area = psgt(data.select.x, param.mu, param.sigma, 
                        param.lambda, param.p, param.q, mean.cent = FALSE))
```
...
task2_df %>% ggplot(aes(y = data.ans.grad, x = param.p)) +
  geom_point(alpha = 0.5)

task2_df %>% ggplot(aes(y = data.ans.grad, x = param.q)) +
  geom_point(alpha = 0.5)

task2_df %>% ggplot(aes(y = data.ans.grad, x = param.lambda)) +
  geom_point(alpha = 0.5)

task2_df %>% ggplot(aes(y = data.ans.grad, x = param.mu)) +
  geom_point(alpha = 0.5)

task2_df %>% ggplot(aes(y = data.ans.grad, x = param.sigma)) +
  geom_point(alpha = 0.5)

task2_df %>% left_join(participants, by = join_by(participantId)) %>% 
  ggplot(aes(y = data.ans.grad, x = dist_to_screen)) +
  geom_point(alpha = 0.5)
```
...
task_df %>% filter(task == "task2") %>% 
  select(-slider.x, -slider.y) %>% 
  left_join(participants, by = join_by(participantId)) %>% 
  mutate(phy.x = pixel_to_phy_x(pixel.select.x, dist_to_screen) - pixel_to_phy_x(data_to_pixel_x(param.mu), dist_to_screen), 
         phy.y = pixel_to_phy_y(pixel.select.y, dist_to_screen) - pixel_to_phy_y(data_to_pixel_x(dsgt(param.mu, param.mu, param.sigma, param.lambda, param.p, param.q, mean.cent = FALSE)), dist_to_screen)) %>%
