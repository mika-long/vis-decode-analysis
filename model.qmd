...
The only thing that is worth noting is the relationship between `data.select.grad` and `dist_to_screen`. 
...
task2_df %>% mutate(diff.x = phy.select.x - phy.ans.x, 
  ...
  mutate(angle = case_when(diff.x == 0 ~ 0, 
                           diff.x != 0 ~ atan(abs(diff.y) / abs(diff.x)) * 180 / pi )) %>% 
  ggplot(aes(x = angle)) + 
  geom_dots()
...
task2_df %>% mutate(diff.x = phy.select.x - phy.ans.x, 
  ...
  mutate(angle = case_when(diff.x == 0 ~ 0, 
                           diff.x != 0 ~ atan(abs(diff.y) / abs(diff.x)) * 180 / pi )) %>% 
  ggplot(aes(x = angle, y = abs(data.select.angle))) + 
  geom_point(alpha = 0.5) + 
  geom_abline(intercept = 0, slope = 1, color = "black", linetype = "dashed")
```

This just means that if we were to use the highest point and the selected point as ways to calculate the actual angle, then it is smaller than the actual slope at the selected point. 
