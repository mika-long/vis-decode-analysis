...
There could also be two versions of this ... one is additive and the other is duplicaative ... 
...
m5.2.1.x <- task5_df %>% mutate(error_phy = phy.select.x - phy.ans.x) %>% 
  brm(formula = f1,
      data = .,
      family = gaussian(),
      prior = p,
...
m5.2.2.x <- task5_df %>% mutate(error_phy = phy.select.x - phy.ans.x) %>% 
  brm(formula = f2,
      data = .,
      family = gaussian(),
      prior = p,
...
m5.1.x <- add_criterion(m5.1.x, "loo")
m5.2.1.x <- add_criterion(m5.2.1.x, "loo", moment_match = TRUE)
...
Based on the result of the LOO comparison, the visual angle model `m5.1.x` is preferred. Based on the model comparison results, `sigma ~ 0 + offset(log(phy.ans.y + dist_to_screen))  + (1 | participantId)` is better than `sigma ~ 0 + offset(log(phy.ans.y)) + offset(log(dist_to_screen))  + (1 | participantId)`.  
...
m5.2.y <- task5_df %>% mutate(error_phy = phy.select.y - phy.ans.y) %>%
  brm(
    formula = f,
    data = .,
    family = gaussian(),
...
```{r}
m5.1.y <- add_criterion(m5.1.y, "loo")
m5.2.y <- add_criterion(m5.2.y, "loo", moment_match = TRUE)
...
```{r}
m3.1 <- add_criterion(m3.1, "loo")
m3.2 <- add_criterion(m3.2, "loo", moment_match = TRUE)
...
Plotting left area against lambda: 
