...
Remember that for this, we need to filter out 
...
task2_df %>% ggplot(aes(y = data.ans.grad, x = dist_to_screen)) +
  geom_point(alpha = 0.5)
```
...
task2_df %>% 
  mutate(diff.x = phy.select.x - phy.ans.x, 
         ...
         angle = atan(abs(diff.y) / abs(diff.x)) * 180 / pi) %>% 
  ggplot(aes(x = angle)) + 
  geom_dots()
```

We are seeing these extreme outliers because `diff.x` is very, very small, almost close to 0. The above calculations does not deal well with very, very small denominators. 
