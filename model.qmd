...
\log(\sigma_{j, i}) &= \beta \cdot \log(\texttt{va.ans.y}_i) + \gamma_j, & j \in [13] \\ 
\gamma_j &\sim \mathcal{N}(\mu_\gamma, \sigma_\gamma), & j \in [13]
\end{aligned}
$$
...
m5.2.3.x <- add_criterion(m5.2.3.x, "loo", moment_match=TRUE, recompile=TRUE)
